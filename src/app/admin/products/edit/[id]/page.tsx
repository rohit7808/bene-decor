...
export default function EditProductPage({ params }: EditProductProps) {
      ...
      try {
        ...
        if (data.success && data.product) {
          ...
          setFormData({
            ...
            originalPrice: String(p.originalPrice !== undefined ? p.originalPrice : ""),
            discount: String(p.discount !== undefined ? p.discount : "0"),
            stock: String(p.stock !== undefined ? p.stock : "10"),
            lowStockThreshold: String(p.lowStockThreshold !== undefined ? p.lowStockThreshold : "5"),
            sku: p.sku || "",
          ...
          if (Array.isArray(p.variants) && p.variants.length > 0) {
            setVariants(
              p.variants.map((v: any, idx: number) => ({
                id: v._id || v.id || String(Date.now() + idx),
                _id: v._id || v.id,
                colorName: v.colorName || "",
                colorCode: v.colorCode || "#A67C52",
                price: String(v.price !== undefined ? v.price : p.price || 0),
                originalPrice: String(v.originalPrice !== undefined ? v.originalPrice : p.originalPrice || 0),
          ...
          setErrorMessage(data.error || "Product not found.");
        }
      } catch (err) {
        console.error("Fetch Product Error:", err);
        setErrorMessage("Failed to load product details from MongoDB.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // Submit product update to PATCH /api/admin/products/[id]
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setSuccessMessage("Product updated successfully! Redirecting to products list...");
        setTimeout(() => {
  ...
  if (isLoading) {
    return (
      <AdminProtectedRoute>
        <div className="min-h-screen bg-[#FAF8F5] text-[#1F1F1F] p-8 flex flex-col items-center justify-center">
          <div className="text-sm font-semibold text-[#A67C52] animate-pulse">
